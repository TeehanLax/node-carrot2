...
SearchResult.prototype.addDocument = function addDocument(title, url, snippet, params) {
...
SearchResult.prototype.toXML = function toXML(options) {
    ...
    for(var i=0; i<length; i++) {
        if(!ids) doc = this.documents[i] || this.documents[ids[i]];
...
SearchResult.prototype.parseOutput = function parseOutput(data) {
    if(data.clusters) {
        ...
        for(var i=0; i<data.clusters.length; i++) {
            ...
            var key = serialize(incoming_cluster.phrases);
            var existing_cluster = this.clusterHash[key];
            ...
            if(existing_cluster) {
                ...
                this.clusterHash[key] = incoming_cluster;
